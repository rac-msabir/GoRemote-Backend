...
class SavedJobController extends Controller
{
    ...
    public function postJobs(Request $request)
    {
        try {
            ...
            // ---------------------------------------------------
            // Validate fields
            // ✅ company_id is treated as Employer ID
            // ✅ if company_id is null => create employer using company_name/website/country_code
            // ---------------------------------------------------
            $validated = $request->validate([
                'title'             => 'required|string|max:191',
                'category_id'       => 'required|integer|exists:categories,id',
                ...
                // ✅ Employer selection (existing)
                'company_id'        => 'nullable|integer|exists:employers,id',

                // ✅ Employer creation (only required if company_id is null)
                'company_name'      => 'required_without:company_id|string|max:255',
                'email'             => 'required_without:company_id|string|max:255',
            ...
            // Persist everything in a transaction
            // ---------------------------------------------------
            return DB::transaction(function () use ($validated, $skills, $benefits, $request, $descRows) {

                // ✅ If company_id is null => create new employer
                // ✅ If company_id is not null => use it as employer_id
                if (!empty($validated['company_id'])) {
                    $employerId = (int) $validated['company_id'];
                } else {
                    // NOTE: Employer model must exist: App\Models\Employer
                ...
                $job = \App\Models\Job::create([
                    ...
                    // ✅ store employer id here
                    'employer_id'     => $employerId,

                    // (keep company_id null unless you really use a separate companies table)
                    'company_id'      => null,
    ...
    public function employeers(Request $request)
    {
        ...
        $query = Employer::query()->orderByDesc('id');
        ...
        return response()->api($companies, true, 'Employers fetched', 200);
    }
    /**
