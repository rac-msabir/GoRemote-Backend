...
class EmployerSeeder extends Seeder
{
    ...
    {
        // Seed baseline job benefits if empty
        $benefits = [
        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            for ($j = 0; $j < $numJobs; $j++) {
                $job = Job::create([
                    'employer_id' => $employer->id,
                    'title' => fake()->jobTitle(),
                    'slug' => Str::slug(fake()->unique()->jobTitle().'-'.uniqid()),
                ...
                shuffle($benefitIds);
                $attachIds = array_slice($benefitIds, 0, rand(2, 5));
                foreach ($attachIds as $benefitId) {
                ...
                $questionTypes = ['text','boolean','number'];
                for ($q = 0; $q < rand(2, 4); $q++) {
